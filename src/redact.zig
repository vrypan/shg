...
pub fn redactCommand(cmd: []const u8, token: []const u8, alloc: std.mem.Allocator) ![]u8 {
    if (token.len == 0) return alloc.dupe(u8, cmd);
    const idx = std.mem.indexOf(u8, cmd, token) orelse return alloc.dupe(u8, cmd);
...
test "redact command" {
    ...
    defer alloc.free(r);
    try std.testing.expectEqualStrings("export API_KEY=sk-...56", r);
}
